function(input, output, session) {
  ...
  observeEvent(input$metric_select, ignoreInit=T, {
    if(input$metric_select=="RR"){
      metric("RR")
    }
    else{
      metric("PRR")
    }
    swap(dset$data$PRR, dset$data$RR)
    swap(dset$data$L2_PRR, dset$data$L2_RR)
    swap(dset$data$L10_PRR, dset$data$L10_RR)
  })
  
  output$box_select <- renderUI({
    selectInput("sort_by", c("Number of Adverse Events", metric(), "Alphabetical"), 
  ...
  output$faers_link <- renderUI({
    ...
    tags$a(href=paste0("https://fis.fda.gov/sense/app/95239e26-e0be-42d9-a960-9a5f7f1c25ee/sheet/45beeb74-30ab-46be-8267-5756582633b4/state/analysis/select/Search%20Term/", gsub(' ', '-', str_to_title(input$intro_drug)), "%20(G)"),
           "View on FAERS Public Dashboard")
  })
  ...
  output$top_ae <- renderPlotly({
    ...
    aes = dset$data[which(dset$data$INAME == input$intro_drug), c('PT_TERM', 'PT_COUNT', 'PRR')] %>% distinct();
    shiny::validate(
    ...
    if(input$sort_by=="Alphabetical"){
      plot_ly(aes, x=~PT_COUNT, y=~PT_TERM, type = "bar", orientation = "h", height = 12*nrow(aes), width="100%",
        ...
        layout(title=input$intro_drug, bargap=.1, showlegend = FALSE, autosize=TRUE, yaxis = list(title="", automargin = TRUE, tickfont = list(size = 8)), 
               xaxis = list(title=metric(), side="top")) %>%
  ...
  output$dload <- downloadHandler(
    ...
    content = function(file){
      if(input$downloadType == ".csv") {
        write.csv(dset$data[which(dset$data$INAME == input$intro_drug), c('PT_TERM', 'PT_COUNT', isolate(metric()))], file, row.names = FALSE)
      } else if(input$downloadType == ".json") {
        exportJSON <- toJSON(dset$data[which(dset$data$INAME == input$intro_drug), c('PT_TERM', 'PT_COUNT', isolate(metric()))])
        write(exportJSON, file)
      } else if(input$downloadType == ".xlsx") {
        write_xlsx(dset$data[which(dset$data$INAME == input$intro_drug), c('PT_TERM', 'PT_COUNT', isolate(metric()))], path=file)
      } else if(input$downloadType == ".txt") {
        write.table(dset$data[which(dset$data$INAME == input$intro_drug), c('PT_TERM', 'PT_COUNT', isolate(metric()))], file, row.names=FALSE)
      }
    }
  ...
  observeEvent(input$popdt, ignoreInit=T, {
    aes = dset$data[which(dset$data$INAME == input$intro_drug), c('PT_TERM', 'PT_COUNT', 'PRR')]
    if(nrow(aes)==0){
    ...
    if(input$sort_by=="Number of Adverse Events"){
      ...
      colnames(df) <- c("Adverse Event", "Count", isolate(metric()))
    }
  ...
  output$pie_chart <- renderPlotly({
    total_rows = nrow(dset$data[which(dset$data$INAME==input$intro_drug),])
    shiny::validate(
      need(total_rows>0, "No Data Available")
    )
    g1 = nrow(dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR<1),])
    g2 = nrow(dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR>=1 & dset$data$PRR<5),])
    g3 = nrow(dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR>=5 & dset$data$PRR<10),])
    g4 = nrow(dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR>=10 & dset$data$PRR<100),])
    g5 = nrow(dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR>=100),])
    df <- data.frame("label"=c("<1", "1-5", "5-10", "10-100", ">100"), "vals" = c(g1, g2, g3, g4, g5))
    plot_ly(df, labels=~label, values=~vals, key=c("<1", "1-5", "5-10", "10-100", ">100"), type="pie", source="E", hovertemplate=paste(metric(), ": %{label} <br> %{value}<extra></extra>"), name="") %>%
      plotly::config(modeBarButtons = list(list("toImage")), displaylogo = FALSE) %>% 
      layout(title=input$intro_drug, legend=list(title=list(text=paste('<b>', metric(), '</b>'))))
  })
  ...
  observeEvent(event_data("plotly_click", source = "E"), {
    ...
    if(pieData$key=="<1") tableDat = dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR<1), c('PT_TERM', 'PT_COUNT', isolate(metric()))]
    else if(pieData$key=="1-5") tableDat = dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR>=1 & dset$data$PRR<5), c('PT_TERM', 'PT_COUNT', isolate(metric()))]
    else if(pieData$key=="5-10") tableDat = dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR>=5 & dset$data$PRR<10), c('PT_TERM', 'PT_COUNT', isolate(metric()))]
    else if(pieData$key=="10-100") tableDat = dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR>=10 & dset$data$PRR<100), c('PT_TERM', 'PT_COUNT', isolate(metric()))]
    else if(pieData$key==">100") tableDat = dset$data[which(dset$data$INAME==input$intro_drug & dset$data$PRR>=100), c('PT_TERM', 'PT_COUNT', isolate(metric()))]
    ...
    output$download_5 <- downloadHandler(
      ...
      filename = function(){
        paste(input$intro_drug, "-related adverse events,", metric(), pieData$key, input$downloadType5)
      },
      content = function(file){
    ...
    showModal(modalDialog(
      size = "l",
      title = paste0(input$intro_drug, "- related adverse events with ", metric(), pieData$key),
  ...
  output$sum_table <- renderDT({
    ...
    dat1 <- dset$data$PRR[which(dset$data$INAME==input$intro_drug)]
    ...
    dat2 <- dset$data$PT_COUNT[which(dset$data$INAME==input$intro_drug)]
    ...
    df <- cbind(data.frame("1" =c(isolate(metric()), "Count"), "2" = c(min_prr, min_cnt), "3"=c(q1_prr, q1_cnt), "4" = c(median_prr, median_cnt), 
                           "5"=c(q3_prr, q3_cnt), "6" = c(max_prr, max_cnt), "7"=c(mean_prr, mean_cnt), "8"=c(std_prr, std_cnt)));
    n=length(unique(dset$data$PT_TERM[which(dset$data$INAME==input$intro_drug)]))
  ...
  output$box_select2 <- renderUI({
    selectInput("sort_by2", c("PT Count", metric(), "Alphabetical"), 
                label = "Sort by", selected="Number of Adverse Events", multiple=FALSE, width = "210px")
  })
  ...
  output$top_subs <- renderPlotly({
    ...
    aes = dset$data[which(dset$data$PT_TERM == input$intro_ae), c('INAME', 'PT_COUNT', 'PRR')] %>% distinct();
  ...
  output$dload2 <- downloadHandler(
    ...
    content = function(file){
      if(input$downloadType6 == ".csv") {
        write.csv(dset$data[which(dset$data$PT_TERM == input$intro_ae), c('INAME', 'PT_COUNT', metric())], file, row.names = FALSE)
      } else if(input$downloadType6 == ".json") {
        exportJSON <- toJSON(dset$data[which(dset$data$PT_TERM == input$intro_ae), c('INAME', 'PT_COUNT', metric())])
        write(exportJSON, file)
      } else if(input$downloadType6 == ".xlsx") {
        write_xlsx(dset$data[which(dset$data$PT_TERM == input$intro_ae), c('INAME', 'PT_COUNT', metric())], path=file)
      } else if(input$downloadType6 == ".txt") {
        write.table(dset$data[which(dset$data$PT_TERM == input$intro_ae), c('INAME', 'PT_COUNT', metric())], file, row.names=FALSE)
      }
    }
  ...
  observeEvent(input$popdt2, ignoreInit=T, {
    aes = dset$data[which(dset$data$PT_TERM == input$intro_ae), c('INAME', 'PT_COUNT', 'PRR')]
    if(nrow(aes)==0){
      showNotification("Not enough data available", type="warning")
    }
    shiny::validate(
    ...
    if(input$sort_by2=="PT Count"){
      ...
      colnames(df) <- c("Substance", "Count", isolate(metric()))
    }
  ...
  output$pie_chart22 <- renderPlotly({
    total_rows = nrow(dset$data[which(dset$data$PT_TERM == input$intro_ae),])
    shiny::validate(
      need(total_rows>0, "No Data Available")
    )
    g1 = nrow(dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR<1),])
    g2 = nrow(dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR>=1 & dset$data$PRR<5),])
    g3 = nrow(dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR>=5 & dset$data$PRR<10),])
    g4 = nrow(dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR>=10 & dset$data$PRR<100),])
    g5 = nrow(dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR>=100),])
    df <- data.frame("label"=c("<1", "1-5", "5-10", "10-100", ">100"), "vals" = c(g1, g2, g3, g4, g5))
    plot_ly(df, labels=~label, values=~vals, key=c("<1", "1-5", "5-10", "10-100", ">100"), type="pie", source="P", hovertemplate=paste(metric(), ": %{label} <br> %{value}<extra></extra>"), name="") %>%
      plotly::config(modeBarButtons = list(list("toImage")), displaylogo = FALSE) %>% 
      layout(title=input$intro_ae, legend=list(title=list(text=paste('<b>', metric(), '</b>'))))
  })
  ...
  observeEvent(event_data("plotly_click", source = "P"), {
    ...
    if(pieData$key=="<1") tableDat = dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR<1), c('INAME', 'PT_COUNT', 'PRR')]
    else if(pieData$key=="1-5") tableDat = dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR>=1 & dset$data$PRR<5), c('INAME', 'PT_COUNT', 'PRR')]
    else if(pieData$key=="5-10") tableDat = dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR>=5 & dset$data$PRR<10), c('INAME', 'PT_COUNT', 'PRR')]
    else if(pieData$key=="10-100") tableDat = dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR>=10 & dset$data$PRR<100), c('INAME', 'PT_COUNT', 'PRR')]
    else if(pieData$key==">100") tableDat = dset$data[which(dset$data$PT_TERM == input$intro_ae & dset$data$PRR>=100), c('INAME', 'PT_COUNT', 'PRR')]
    ...
    output$download_7 <- downloadHandler(
      filename = function(){
        paste(input$intro_ae, "-related substances,", metric(), pieData$key, input$downloadType7)
      },
      content = function(file){
  ...
  output$sum_table2 <- renderDT({
    ...
    #mean, median, standard deviation, count, iqr
    dat1 <- dset$data$PRR[which(dset$data$PT_TERM == input$intro_ae)]
    q1_prr <- round(quantile(dat1)[2], 2)
    median_prr = round(median(dat1), 2)
    q3_prr <- round(quantile(dat1)[4], 2)
    mean_prr <- round(mean(dat1), 2)
    ...
    dat2 <- dset$data$PT_COUNT[which(dset$data$PT_TERM == input$intro_ae)]
    q1_cnt <- round(quantile(dat2)[2], 2)
    median_cnt <- round(median(dat2), 2)
    q3_cnt <- round(quantile(dat2)[4], 2)
    mean_cnt <- round(mean(dat2), 2)
    ...
    df <- cbind(data.frame("1" =c(isolate(metric()), "Count"), "2" = c(min_prr, min_cnt), "3"=c(q1_prr, q1_cnt), "4" = c(median_prr, median_cnt), 
                           "5"=c(q3_prr, q3_cnt), "6" = c(max_prr, max_cnt), "7"=c(mean_prr, mean_cnt), "8"=c(std_prr, std_cnt)));
    if(length(dat1)>0){
      n=dset$data$CASE_COUNT[which(dset$data$PT_TERM == input$intro_ae)][1]
    }
    else{
      n=0
    }
  ...
  output$mult_two_ae <- renderPlot({
    ...
    pt1 <- subset(dset$data, select=c(INAME,CASE_COUNT, PT_COUNT, PT_TERM, PRR, L10_PRR, ATC1, ATC2, ATC3, ATC4),
                  subset=(PT_TERM == input$ae1 & CASE_COUNT>ifelse(is.null(input$num_obs),1000,input$num_obs)))
    pt2 <- subset(dset$data, select = c(INAME, CASE_COUNT, PT_COUNT, PT_TERM, PRR, L10_PRR, ATC1, ATC2, ATC3, ATC4),
    ...
    ggplot(data, aes(x = L10_PRR.x, y = L10_PRR.y))+
      ...
      labs(
        ...
        subtitle = paste(metric(), "Values are Log-Transformed in Both Axes")
      ) +
      theme_bw() +
  ...
  observeEvent(event_data("plotly_click", source = "Aa"),  {
    ...
    pt1 <- subset(dset$data, select = c(INAME,CASE_COUNT, PT_COUNT, PRR, ATC1, ATC2, ATC3, ATC4),
                  subset=(PT_TERM == input$ae1 & col %in% list & CASE_COUNT>cc_1() & PT_COUNT>ptc_1()))
    pt2 <- subset(dset$data, select = c(INAME, PT_COUNT, PRR),
    ...
    colnames(comb) = c("SUBSTANCE", "SUBSTANCE COUNT", paste(input$ae1, " COUNT"), paste(barData$x, " COUNT"), paste(input$ae1, metric()), 
  ...
  rerender <- function() {
    ...
    for(ae in input$other_ae){
      pt1 <- subset(dset$data, select = c(INAME, CASE_COUNT, PT_COUNT, PT_TERM, L10_PRR, ATC1),
                    subset=(PT_TERM == input$ae1 & col %in% list & CASE_COUNT>cc_1() & PT_COUNT>ptc_1()))
      pt2 <- subset(dset$data, select = c(INAME, CASE_COUNT, PT_COUNT, PT_TERM, L10_PRR, ATC1),
  ...
  observeEvent(input$other_ae, {
    ...
    if(dim(pts$data)[1] < length(ae_list)){
      ...
      pt1 <- subset(dset$data, select=c(INAME,CASE_COUNT, PT_COUNT, PT_TERM, L10_PRR, ATC1, ATC2, ATC3, ATC4),
                    subset=(PT_TERM == input$ae1 & CASE_COUNT>cc_1() & PT_COUNT>ptc_1()))
      pt2 <- subset(dset$data, select = c(INAME, CASE_COUNT, PT_COUNT, PT_TERM, L10_PRR, ATC1, ATC2, ATC3, ATC4),
  ...
  output$table1<-renderDataTable({
    ...
    dt <- datatable(ord_subset(), 
                    selection = "single", 
                    rownames = FALSE,
                    colnames = c("SUBSTANCE", "SUBSTANCE COUNT", paste(x, " COUNT"), paste(x, metric()), paste(y, " COUNT"), paste(y, metric()),
...
<script>lastClicked = ""; </script>
  ...
  rerender_subs <- function(){
    ...
    for(sub in input$sub2){
      sub1 <- subset(dset$data, select=c(PT_TERM, PT_COUNT, PT_TERM, PRR, L10_PRR),
                     subset=(INAME == input$sub1 & PT_COUNT>input$min_ae))
      sub2 <- subset(dset$data, select=c(PT_TERM, PT_COUNT, PT_TERM, PRR, L10_PRR),
  ...
  output$mult_two_subs <- renderPlot({
    ...
    d1<-subset(dset$data,select=c(PT_TERM,PT_COUNT,PT_TERM,PRR,L10_PRR),
               subset=(INAME==input$sub1))
    d2<-subset(dset$data,select=c(PT_TERM,PT_COUNT,PT_TERM,PRR,L10_PRR),
  ...
  output$table2 <- renderDataTable({
    ...
    datatable(getSubset2(), 
              selection = "single", 
              rownames = FALSE,
              colnames = c("Adverse Event", paste(x, " Count"), paste(x, metric()), paste(y, " Count"), paste(y, metric())),
              options = list(
                lengthMenu = list(c(10, 25, 50, -1), c("10", "25", "50", "All")),
                autoWidth = TRUE,
                scrollX = TRUE,
  ...
  output$download2 <- downloadHandler(
    ...
    content = function(file){
      write.csv(dset$data, file, sep=",", row.names = FALSE)
    }
  )
  ...
  class_dat_global <- reactive({
    if(input$class_op=="User-selected drugs"){
      data <- dset$data[which(dset$data$INAME %in% input$custom_list),]
    }
    else if(input$class_op=="ATC Class") {
      req(!is.null(input$cc_1))
      if(str_sub(as.character(input$cc_1), 1, 1)=="*") class=str_sub(as.character(input$cc_1), 2)
      else class=input$cc_1
      if(input$cc_level=="1") data<-unique(dset$data[which(dset$data$ATC1==class),])
      else if(input$cc_level=="2") data<-unique(dset$data[which(dset$data$ATC2==class),])
      else if(input$cc_level=="3") data<-unique(dset$data[which(dset$data$ATC3==class),])
      else if(input$cc_level=="4") data<-unique(dset$data[which(dset$data$ATC4==class),])
    }
    else{
      ...
      if(tolower(tools::file_ext(infile$datapath)) == "xlsx"){
        tryCatch(
          {
            drugs <- read.xlsx(input$drugs_file$datapath,
                               ...
                               colNames=F,
                               cols=1)[,1]
            data <- dset$data[which(dset$data$INAME %in% drugs),]
          },
          error = function(e) {
        ...
        tryCatch(
          {
            data <- dset$data[which(dset$data$INAME %in% readLines(input$drugs_file$datapath)),]
          },
          error = function(e) {
            # return a safeError if a parsing error occurs
            stop(safeError(e))
  ...
  dat4 <- reactive({
    ...
    d2<-subset(dset$data,select=c(PT_TERM,PT_COUNT,PRR),
  ...
  observeEvent(input$cc_2, ignoreInit=T, {
    if(!(input$cc_2 %in% dset$data$INAME)){
  ...
  output$cc_1 <- renderUI({
    if(length(dset$data$ATC1[which(dset$data$INAME==input$cc_2)])>1){
      excl=dset$data[which(dset$data$INAME==input$cc_2)[1],]
    }
    else{
      excl=dset$data[which(dset$data$INAME==input$cc_2),]
    }
    if(input$cc_level=="2"){
  ...
  output$bar4 <- renderPlotly({
    ...
    if(input$sortby4=="Class"){
      plot_ly(dat4(), x=~reorder(PT_TERM, -PRR.x), y=~PRR.x, type="bar", name="Class", 
        ...
        layout(bargap=.1, yaxis = list(title = isolate(metric()), showline = TRUE), xaxis = list(title='Adverse Events', tickangle=90, showline = TRUE), 
  ...
  output$boxplots <- renderPlotly({
    ...
    dat = rbind(dat, dset$data[which(dset$data$INAME==input$cc_2),])
    ...
    plot_ly(dat, x = ~factor(INAME, levels), y=~L10_PRR, type = "box", source='H', width= max(max(20*n, 1000), session$clientData$output_bar4_width), 
      ...
      layout(
        margin = list(b=margin_size),
        yaxis = list(title=paste("log-10", metric())),
  ...
  output$drugperc <- renderDataTable(rownames=FALSE, selection="single", class="compact",
    data.frame(setNames(list(round(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)),])$PRR, 2), 
                        round(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)),])$L10_PRR, 2), 
                        paste0(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)),])$PT_COUNT, "/", unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)),])$CASE_COUNT, " (", round(as.numeric(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)),])$PT_COUNT)/as.numeric(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)),])$CASE_COUNT), digits=3), ")"), 
                        unique(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100))])
    ), c(metric(), "LOG_10", "SUBS.", "ADVERSE_EVENT")))
  )
  ...
  output$dload4_1 <- downloadHandler(
    ...
    content = function(file){
      if(input$downloadType4_1 == ".csv") {
        write.csv(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, .95))], file, row.names = FALSE)
      } else if(input$downloadType4_1 == ".json") {
        exportJSON <- toJSON(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, .95))])
        write(exportJSON, file)
      } else if(input$downloadType4_1 == ".xlsx") {
        write_xlsx(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, .95))], path=file)
      } else if(input$downloadType4_1 == ".txt") {
        write.table(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, .95))], file, row.names=FALSE)
      }
    }
  )
  ...
  output$dload42 <- downloadHandler(
    ...
    content = function(file){
      if(input$downloadType42 == ".csv") {
        write.csv(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, .9))], file, row.names = FALSE)
      } else if(input$downloadType42 == ".json") {
        exportJSON <- toJSON(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, .9))])
        write(exportJSON, file)
      } else if(input$downloadType42 == ".xlsx") {
        write_xlsx(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, .9))], path=file)
      } else if(input$downloadType42 == ".txt") {
        write.table(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, .9))], file, row.names=FALSE)
      }
    }
  ...
  observeEvent(input$drugperc_rows_selected, ignoreNULL = FALSE, {
    x_val(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)),])$L10_PRR[input$drugperc_rows_selected])
  ...
  output$percentile <- renderText({
    ...
    ae=unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)), "PT_TERM"])[input$drugperc_rows_selected]
  ...
  output$histogram <- renderPlotly({
    ...
    plot_ly(dat, type="histogram", x=~L10_PRR, marker=list(color="peachpuff"), source="I") %>%
      layout(
        ...
        title=paste0(class_name(), "<br>Frequency of ", metric(), "s"),
        showlegend = FALSE,
        xaxis=list(title=paste(metric(), "(log-10)"))
      ) %>%
      plotly::config(modeBarButtons = list(list("toImage")), displaylogo = FALSE)
  })
  ...
  output$perc_count <- renderUI({
    ...
    len <- length(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input/100)),])$PT_TERM)
    paste("Count: ", len, " out of ", length(unique(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2)])))
  })
  ...
  output$drugperc21 <- renderDataTable(rownames=FALSE, selection="single", class="compact",
    data.frame(setNames(list(round(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)),])$PRR, 2),
                        round(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)),])$L10_PRR, 2),
                        paste0(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)),])$PT_COUNT, "/", unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)),])$CASE_COUNT, " (", round(as.numeric(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)),])$PT_COUNT)/as.numeric(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)),])$CASE_COUNT), digits=3), ")"),
                        unique(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100))])),
                  c(metric(), "LOG_10", "SUBS.", "ADVERSE_EVENT")))
    
  )
  
  observeEvent(input$drugperc21_rows_selected, ignoreNULL = FALSE, {
    x_val21(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)),])$L10_PRR[input$drugperc21_rows_selected])
  ...
  output$percentile21 <- renderText({
    ...
    ae=unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)), "PT_TERM"])[input$drugperc21_rows_selected]
    paste(ae, " > ", round(x_val21(), 2), " corresp. to < ", round(100*ptile, 2), "%")
  })
  ...
  output$perc_count21 <- renderUI({
    ...
    dat<-class_dat_global()$PRR
    len <- length(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input21/100)),])$PT_TERM)
    paste("Count: ", len, " out of ", length(unique(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2)])))
  })
  # END hist 1
  ...
  class2_dat_global <- reactive({
    if(input$class_op2=="User-selected drugs"){
      data <- dset$data[which(dset$data$INAME %in% input$custom_list2),]
    }
    else if(input$class_op2=="ATC Class") {
      req(!is.null(input$cc_class2))
      if(str_sub(as.character(input$cc_class2), 1, 1)=="*") class=str_sub(as.character(input$cc_class2), 2)
      else class=input$cc_class2
      if(input$cc_level2=="1") data<-unique(dset$data[which(dset$data$ATC1==class),])
      else if(input$cc_level2=="2") data<-unique(dset$data[which(dset$data$ATC2==class),])
      else if(input$cc_level2=="3") data<-unique(dset$data[which(dset$data$ATC3==class),])
      else if(input$cc_level2=="4") data<-unique(dset$data[which(dset$data$ATC4==class),])
    }
    else{
      ...
      if(tolower(tools::file_ext(infile$datapath)) == "xlsx"){
        ...
        tryCatch(
          {
            data <- dset$data[which(dset$data$INAME %in% readLines(input$drugs_file2$datapath)),]
          },
          error = function(e) {
            # return a safeError if a parsing error occurs
            stop(safeError(e))
  ...
  output$combined_hist <- renderPlotly({
    ...
    fig <- subplot(fig1, fig2, shareY = TRUE)%>%  
      layout(title=paste0("Frequency of ", metric(), "s"), xaxis=list(title=paste(metric(), "(log-10)")), 
             annotations = list( 
               list(x=.05, y = 1.0, text = class_name(), showarrow = F, xref='paper', yref='paper'), 
               list(x=.9, y = 1.0, text = class_name2(), showarrow = F, xref='paper', yref='paper')
             ) 
      )
  ...
  output$drugperc22 <- renderDataTable(rownames=FALSE, selection="single", class="compact",
     data.frame(setNames(list(round(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input22/100)),])$PRR, 2),
        round(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input22/100)),])$L10_PRR, 2),
        paste0(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input22/100)),])$PT_COUNT, "/", unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input22/100)),])$CASE_COUNT, " (", round(as.numeric(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input22/100)),])$PT_COUNT)/as.numeric(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input22/100)),])$CASE_COUNT), digits=3), ")"),
        unique(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class_dat_global()$PRR, input$pcentile_input22/100))])),
        c(metric(), "LOG_10", "SUBS.", "ADVERSE_EVENT")))
  )
  
  observeEvent(input$drugperc22_rows_selected, ignoreNULL = FALSE, {
    x_val22(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class2_dat_global()$PRR, input$pcentile_input22/100)),])$L10_PRR[input$drugperc22_rows_selected])
  ...
  output$percentile22 <- renderText({
    ...
    ae=unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class2_dat_global()$PRR, input$pcentile_input22/100)), "PT_TERM"])[input$drugperc22_rows_selected]
    paste(ae, " > ", round(x_val22(), 2), " corresp. to < ", round(100*ptile, 2), "%")
  })
  ...
  output$perc_count22 <- renderUI({
    ...
    len <- length(unique(dset$data[which(dset$data$INAME==input$cc_2 & dset$data$PRR>=quantile(class2_dat_global()$PRR, input$pcentile_input22/100)),])$PT_TERM)
    paste("Count: ", len, " out of ", length(unique(dset$data$PT_TERM[which(dset$data$INAME==input$cc_2)])))
  })
  ...
  observeEvent(input$class_choices, { 
    x=unique(dset$data$INAME[which(dset$data$ATC1==input$class_choices |  dset$data$ATC2==input$class_choices | dset$data$ATC3==input$class_choices | dset$data$ATC4==input$class_choices)])
    process(x)
  })
  ...
  process <- function(drugs){
    ...
    for(ptt in unique(dset$data$PT_TERM)){
      prrs=c()
      for(drug in drugs){
  ...
  output$grouped <- renderPlot({
    ...
    classes=unique(dset$data$ATC1[which(dset$data$INAME %in% rownames(dat))])
    ...
    for(class in classes){
      for(i in 1:input$c){
        ...
        for(j in 1:length(rownames(dat))){
          if(dset$data$ATC1[which(dset$data$INAME==rownames(dat)[j])]==class && grpDat$cluster[j]==i) cnt=cnt+1;
        }
  ...
  output$filtered_bar <- renderPlotly({
    ...
    df <- data.frame("Date"=as.Date(dates), "Reports"=count)
    
    fig <- plot_ly(df, type = 'scatter', mode = 'lines', width="100%")%>%
      add_trace(x = ~Date, y = ~Reports, name = paste(substance, "Report Count"))%>%
      layout(showlegend = F)
    options(warn = -1)
